...
Consider the space $X=(X,d)$ where $d$ is the discrete metric. Then the 
\end{exercise}

\begin{exercise}{1.3.14}
Assume that $T\colon X\rightarrow Y$ is continuous. Let $M\subset Y$ and $T^{-1}(M)$ be the inverse image of $M$. Let $M$ be closed. Then $M^C$ is open. Since $T$ is continuous, $T^{-1}(M^C)$ is also open. Let $m\in T^{-1}(M)^C$. Then $T(m)\in M^c$. So $T^{-1}(M)^C\subset T^{-1}(M^C)$. Since $T^{-1}(M^C)$ is open, then $T^{-1}(M)^C$ is open. So $T^{-1}(M)$ is closed.
