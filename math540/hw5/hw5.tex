...
\[d((x_n),(x_m))=\sup_i|(x_{n_i}-x_{m-i}|=|\frac{1}{m}|<\frac{1}{N}=\epsilon\]. So $(x_n)$ is Cauchy. However each entry is made up of entries in $\mathbb{R}$ which is complete. So $1+\frac{1}{n}$ converges. Namely, as $n\rightarrow\infty$, $1+\frac{1}{n}\rightarrow 1$. So as $n\rightarrow\infty$, $(x_n)\rightarrow(1,1,...)$. This is not a sequence with finitely many nonzero terms, in fact it has infinite. Thus, $(x_n)$ converges to a sequence not in $M$, so it does not converge. So $M$ is not complete.
\end{exercise}
