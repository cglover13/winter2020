...
\end{align}
By way of contradiction, assume that not all $\alpha_i$ are 0 for some $t^*$. So $\alpha_i\neq 0$. Then
\[\alpha_1t^*+\alpha_2(t^*)^2+\dotsb+\alpha_n(t^*)^n=0.\]
Solving for $\alpha_i$, we get that $\alpha_i=-(\sum_{j=1\\j\neq i}^{n-1}\alpha_{j+1}(t^*)^j$. However, then $\alpha_i$ has a different value for a different $t$. Since the constants $\alpha_i$ cannot change, this is a contradiction.
Hence, $\{x_1,...,x_n\}$ is linearly independent.
\end{exercise}
