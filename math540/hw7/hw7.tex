...
\end{align}
By way of contradiction, assume that not all $\alpha_i$ are 0 for some $t^*$. Then
\[\alpha_1t^*+\alpha_2(t^*)^2+\dotsb+\alpha_n(t^*)^n=0.\]
Solving for $\alpha_1$, we get that $\alpha_1=-(\sum_{i=1}^{n-1}\alpha_{i+1}(t^*)^i$. However, then $\alpha_1$ has a different value for a different $t$. Since the constants $\alpha_i$ cannot change, this is a contradiction.
