...
Let $X$ be a normed space where absolute convergence implies convergence. Let $(x_n)\in X$ be a Cauchy sequence. Then for $\epsilon^*>0$, there exists an $N$ such that when $m,n>N$, then $\|x_n-x_m\|<\epsilon^*$. We then define $s_k=\sum_{i=1}^k\|x_n-x_{i+m}\|$. Let $\epsilon^*=\frac{\epsilon}{k}$ where $\epsilon>0$. Then for some $K$, when $k>K$, $\|s_k-0\|=\| \sum_{i=1}^k\|x_n-x_{i+m}\|\|<\|\sum_{i=1}^k\epsilon^*\|=\epsilon$. So $s_k$ converges to 0. This means that the sequence $\hat{s}_k=\sum_{i=1}^kx_n-x_{i+m}$ converges to some $x$. This means that for some $\hat{\epsilon}>0$, there exists $M$ such that when $k>M$,
\begin{align}
\|\sum_{i=1}^kx_n-x_{i+m}-x\|&=\|x_n-x-\sum_{i=1}^kx_{i+m}\|\\
&<\hat{\epsilon}.
\end{align}
\end{exercise}
